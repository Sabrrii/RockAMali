#ifndef _DATA_SEND_ASIO_
#define _DATA_SEND_ASIO_
...
class CDataSend_ASIO : public CDataBuffer<Tdata, Taccess>
{
public:
  ...
  CDataSend_ASIO(std::vector<omp_lock_t*> &lock, std::string ip, unsigned short port, boost::uint64_t wait) : CDataBuffer<Tdata, Taccess>(lock)
    , socket(io_service, udp::endpoint(udp::v4(), 0))
    , target(boost::asio::ip::address::from_string(ip), port)
    , wait(wait)
  {
//    this->debug=true;
    this->class_name="CDataSender_ASIO";
