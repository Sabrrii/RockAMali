...
udp_send_run:
	/sbin/ifconfig eth6 | grep TX | grep dropped; ./udp_send -s `echo $(FRAME_SIZE)*4 | bc` -i $(DST_IP) -p $(PORT) -n `echo $(NS)+1 | bc` -w 92 --do-warmup; /sbin/ifconfig eth6 | grep TX | grep dropped
