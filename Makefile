...
udp_receive_run:
	/sbin/ifconfig $(ETH) | grep RX | grep dropped; ethtool -S $(ETH) | grep InDropped --color; env GOMP_CPU_AFFINITY=$(CPU_AFFINITY) ./udp_receive -s `echo $(FRAME_SIZE)*4 | bc` -i $(DST_IP) -p $(PORT) -n $(NS) $(DO_CHECK) --do-warmup --crc false 2>&1 | tee udp_receive.txt; ncdump -h udp_receive.nc > udp_receive.cdl; /sbin/ifconfig $(ETH) | grep RX | grep dropped; ethtool -S $(ETH) | grep InDropped --color
	@echo "sync; make && make udp_receive_run 2>&1 | tee udp_receive.txt"
