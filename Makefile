...
udp_receive_run:
	/sbin/ifconfig enp1s0 | grep RX | grep dropped; ./udp_receive -s `echo $(FRAME_SIZE)*4 | bc` -i $(DST_IP) -p $(PORT) -n $(NS) --do-warmup --verbose --debug; /sbin/ifconfig enp1s0 | grep RX | grep dropped
	@echo "sync; make && make udp_receive_run 2>&1 | tee udp_receive.txt"
...
udp_send_run:
	/sbin/ifconfig eth6 | grep TX | grep dropped; ./udp_send -s `echo $(FRAME_SIZE)*4 | bc` -i $(DST_IP) -p $(PORT) -n `echo $(NS)+1 | bc` -w 321 --do-warmup --do-ramp --verbose; /sbin/ifconfig eth6 | grep TX | grep dropped
