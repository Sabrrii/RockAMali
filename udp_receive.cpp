...
//    std::cout<<"  NcTypeInfo."<<NETCDF_TYPE_INFO_VERSION;
  ...
  switch(id)
  {
    ...
    {//watchdog
      ...
      nci.setNetCDFFile(nc.pNCFile);//share file
      ...
      nci.setNetCDFDims(nc.vpNCDim,nc.pNCDimt);//share dims
      ...
      for(;;)
      {
        ...
        if(i>0)
        {
          //add rate, last index and received counter in NetCDF
          nc_img(0)=rate;
          nc_imgs[0](0)=i;
          nc_imgs[1](0)=count;
          for(int d=0;d<count;++d)
          {
            ...
            nci.addNetCDFData(nc_imgs,nc.loadDimTime()-1);
          }//plateau
        }
#endif //NetCDF
        sleep(twait);
...
/**/
    ...
    //while(1)
      ...
      if(!do_netcdf)
      {
        ...
        nc_imgs[0](0)=index;nc_imgs[1](0)=inc;nc.addNetCDFData(nc_imgs);
